...
void Metric::commit_values_by_updating() {
  if(this->data_vector.size() > 0) {
    this->update_values(this->data_vector);
    this->data_vector.clear();
  }
}
...
int Metric::update_values(const std::vector<std::vector<std::string>> &vector_of_values) {
  this->d.replace_rows_to_table(this->database_name, this->table_name, this->db_columns, vector_of_values);
  return 0;
}
